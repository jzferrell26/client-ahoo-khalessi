...
| ~~W-P7~~ | **RESOLVED 2026-08-26.** The `[LICENSED STATES]` placeholder is gone from `src/` entirely. The footer now links to EMC's own maintained page at `emortgagecapital.com/licensing-and-disclosures` (title: "Our Licensed States"), which is more durable than an inline list because EMC's footprint changes without this repository knowing. The NMLS verification link was also upgraded from the generic homepage to EMC's entity page (COMPANY/1416824). Still worth an EMC nod that linking satisfies their disclosure requirement. | Done | Jonathan |
